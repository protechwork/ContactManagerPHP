...
if (!empty($comment) )
{
    ...
    if($visibility == 1)
    {
        ...
        send_email($emailCredentails["email"], $emailCredentails["email_pasword"], $contactEmail, "Ticket Status", $bodyData);    
    }
...
function getContactEmailByTicketId($ticketID)
{
    ...
    $query = "SELECT 
                ...
                FROM 
                contacts cont 
                INNER JOIN agent_login log ON log.contact_id = cont.contact_id 
                INNER JOIN ticket tkt ON tkt.reported_by = log.id 
...
function getEmailDetailsByAgentID($AgentID)
{
    ...
    $query = "SELECT email, email_pasword FROM agent_notification WHERE agent_id=".$AgentID;
    $result = $mysqli->query($query);
    $returnData = array();
...
function send_email($EmailID, $Password, $To, $Subject, $BodyContent)
{
